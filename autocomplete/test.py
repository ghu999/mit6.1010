...
def test_get_2():
    ...
    try: 
        ...
        print("error not correctly found")
...
def test_contains_2():
    ...
    tree['bar'] = 3
    correct = False
    if 'bar' in tree and 'barking' not in tree and 'ba' not in tree:
        correct = True
    assert correct == True
def test_iter():
    t = lab.PrefixTree()
...
def test_word_frequencies2():
    l = lab.word_frequencies("bat bar ban bank.")
    result = {}
    for key, val in l:
        result[key] = val
    assert result == {'bat':1, 'bar':1, 'ban':1, 'bank':1}
...
def test_autocomplete_2():
    tree = lab.PrefixTree()
    tree['bat'] = 2
    tree['bar'] = 1
    tree['bark'] = 1
    assert {"bat"} == set(lab.autocomplete(tree, "ba", 1))
    assert set() == set(lab.autocomplete(tree, "be", 2))
    assert {'bat', 'bar', 'bark'} == set(lab.autocomplete(tree, "b"))
    assert {'bar', 'bark'} == set(lab.autocomplete(tree, "bar", 2))
...
def test_autocorrect_2():
    tree = lab.PrefixTree()
    tree['bat'] = 2
    tree['bar'] = 1
    tree['bark'] = 1
    tree['boo'] = 10
    result = lab.autocorrect(tree, "bar", 3)
    assert set(result) == {'bar', 'bark', 'bat'}
    result = lab.autocorrect(tree, 'b', 4)
    assert set(result) == {'bar', 'bark', 'bat', 'boo'}
